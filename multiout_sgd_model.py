...
def multiout_sgd_model(train, val, test, outfile):
    # Open the output file for writing
    with open(outfile, 'w') as f:
        # Vectorize separately using TF-IDF for titles and summaries
        title_vectorizer = TfidfVectorizer(max_features=30000, stop_words='english', ngram_range=(1, 2), min_df=2)
        summary_vectorizer = TfidfVectorizer(max_features=30000, stop_words='english', ngram_range=(1, 2), min_df=2)
        ...
        # Define and fit the MultiOutputClassifier with SGDClassifier as the base estimator
        clf = MultiOutputClassifier(SGDClassifier(max_iter=1000, tol=1e-3, random_state=42, n_jobs=-1))
